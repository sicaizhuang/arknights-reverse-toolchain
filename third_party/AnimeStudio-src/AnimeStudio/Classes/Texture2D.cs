...
namespace AnimeStudio
{
    ...
    public class GLTextureSettings
    {
        ...
        public int m_WrapV;
        public int m_WrapW;
        public long m_FilterModeOffset, m_AnisoOffset, m_MipBiasOffset;
        public long m_WrapUOffset, m_WrapVOffset, m_WrapWOffset;
        public string m_FilterModeRawHex, m_AnisoRawHex, m_MipBiasRawHex;
        public string m_WrapURawHex, m_WrapVRawHex, m_WrapWRawHex;

        private static (int value, long offset, string rawHex) ReadIntEvidence(ObjectReader reader)
        {
            var offset = reader.Position - reader.byteStart;
            var raw = reader.ReadBytes(4);
            var value = reader.Endian == EndianType.BigEndian
                ? System.Buffers.Binary.BinaryPrimitives.ReadInt32BigEndian(raw)
                : System.Buffers.Binary.BinaryPrimitives.ReadInt32LittleEndian(raw);
            return (value, offset, Convert.ToHexString(raw));
        }

        private static (float value, long offset, string rawHex) ReadFloatEvidence(ObjectReader reader)
        {
            var offset = reader.Position - reader.byteStart;
            var raw = reader.ReadBytes(4);
            var bits = reader.Endian == EndianType.BigEndian
                ? System.Buffers.Binary.BinaryPrimitives.ReadInt32BigEndian(raw)
                : System.Buffers.Binary.BinaryPrimitives.ReadInt32LittleEndian(raw);
            return (BitConverter.Int32BitsToSingle(bits), offset, Convert.ToHexString(raw));
        }
        ...
        public GLTextureSettings(ObjectReader reader)
        {
            ...
            var filter = ReadIntEvidence(reader);
            m_FilterMode = filter.value; m_FilterModeOffset = filter.offset; m_FilterModeRawHex = filter.rawHex;
            var aniso = ReadIntEvidence(reader);
            m_Aniso = aniso.value; m_AnisoOffset = aniso.offset; m_AnisoRawHex = aniso.rawHex;
            var mip = ReadFloatEvidence(reader);
            m_MipBias = mip.value; m_MipBiasOffset = mip.offset; m_MipBiasRawHex = mip.rawHex;
            ...
            {
                var wrapU = ReadIntEvidence(reader);
                var wrapV = ReadIntEvidence(reader);
                var wrapW = ReadIntEvidence(reader);
                m_WrapMode = wrapU.value;
                m_WrapU = wrapU.value; m_WrapUOffset = wrapU.offset; m_WrapURawHex = wrapU.rawHex;
                m_WrapV = wrapV.value; m_WrapVOffset = wrapV.offset; m_WrapVRawHex = wrapV.rawHex;
                m_WrapW = wrapW.value; m_WrapWOffset = wrapW.offset; m_WrapWRawHex = wrapW.rawHex;
            }
            else
            {
                var wrap = ReadIntEvidence(reader);
                m_WrapMode = wrap.value; m_WrapU = wrap.value; m_WrapV = wrap.value; m_WrapW = wrap.value;
                m_WrapUOffset = wrap.offset; m_WrapURawHex = wrap.rawHex;
                m_WrapVOffset = wrap.offset; m_WrapVRawHex = wrap.rawHex;
                m_WrapWOffset = wrap.offset; m_WrapWRawHex = wrap.rawHex;
            }
