...
namespace AnimeStudio
{
    public sealed class PPtr<T> : IYAMLExportable where T : Object
    {
        ...
        // Preserve the serialized-file external table entry in read-only JSON
        // probes. This lets a PathID reference be followed to its exact CAB
        // without guessing from names or loading unrelated bundles.
        public PPtrExternalReference External
        {
            get
            {
                if (m_FileID <= 0 || m_FileID > assetsFile.m_Externals.Count)
                    return null;
                var external = assetsFile.m_Externals[m_FileID - 1];
                return new PPtrExternalReference
                {
    ...
    public sealed class PPtrExternalReference
    {
